# ⭐ Star Patterns in C++

A curated collection of classic star patterns implemented using nested loops in C++.

These patterns help build a strong foundation in:

- Nested Loops
- Space Handling
- Pattern Observation
- Logic Building
- Competitive Programming Fundamentals

---

## ⭐ 01. Left Half Pyramid

```text
*
**
***
****
*****
```

---

## ⭐ 02. Inverted Left Half Pyramid

```text
*****
****
***
**
*
```

---

## ⭐ 03. Right Half Pyramid

```text
    *
   **
  ***
 ****
*****
```

---

## ⭐ 04. Inverted Right Half Pyramid

```text
*****
 ****
  ***
   **
    *
```

---

## ⭐ 05. Full Pyramid

```text
    *
   ***
  *****
 *******
*********
```

---

## ⭐ 06. Inverted Full Pyramid

```text
*********
 *******
  *****
   ***
    *
```

---

## ⭐ 07. Diamond Pattern

```text
    *
   ***
  *****
 *******
*********
 *******
  *****
   ***
    *
```

---

## ⭐ 08. Hollow Square

```text
*****
*   *
*   *
*   *
*****
```

---

## ⭐ 09. Hollow Rectangle

```text
*******
*     *
*     *
*******
```

---

## ⭐ 10. Hollow Pyramid

```text
    *
   * *
  *   *
 *     *
*********
```

---

## ⭐ 11. Hollow Diamond

```text
    *
   * *
  *   *
 *     *
  *   *
   * *
    *
```

---

## ⭐ 12. Butterfly Pattern

```text
*      *
**    **
***  ***
********
***  ***
**    **
*      *
```

---

## ⭐ 13. X Pattern

```text
*   *
 * *
  *
 * *
*   *
```

---

## ⭐ 14. Hourglass Pattern / Sandglass Pattern

```text
*****
 ***
  *
 ***
*****
```

---

## ⭐ 15. K Pattern

```text
*    *
*   *
*  *
***
*  *
*   *
*    *
```

---

## ⭐ 16. Rhombus Pattern

```text
    *****
   *****
  *****
 *****
*****
```

---

## ⭐ 17. Hollow Rhombus

```text
    *****
   *   *
  *   *
 *   *
*****
```

---

## ⭐ 18. Plus Pattern

```text
  *
  *
*****
  *
  *
```

---

## ⭐ 19. Zebra Pattern

```text
*****
-----
*****
-----
*****
```

---

## ⭐ 20. Zig-Zag Star Pattern

```text
*       *
  *   *
    *
  *   *
*       *
```

---

## 🧠 Concepts Covered

- Nested Loops
- Row & Column Logic
- Space Management
- Symmetry
- Hollow Patterns
- Mathematical Pattern Observation

---

## 🎯 Learning Outcome

After completing all patterns in this repository, you will be comfortable with:

- Loop Visualization
- Pattern-Based Problem Solving
- Competitive Programming Basics
- Grid and Matrix Thinking
- Debugging Nested Loops

---

## 💻 Language

- C++17

---

## 🚀 Author

**kmnayeem**

> Competitive Programmer | Problem Solver
